...
const SheetEditForm: React.FC = () => {
  const { isOpen, closeEditForm, entityType, entityData } = useEditFormContext();
...
const handleSubmit = async (data: any) => {
  if (!entityType) return; // Pastikan `entityType` tersedia

  try {
    // Gunakan `pluralize` untuk membentuk URL yang benar
    const response = await fetch(`/api/${pluralize(entityType)}/${entityData.id}`, {
      method: "PUT",
    ...
    closeEditForm();
    showNotification("Data updated succesfully.");
  } catch (error) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={closeEditForm}>
      <SheetContent className="bg-[#14141c] border-none">
        <SheetHeader>
          <SheetTitle className="text-white">Edit {entityType.charAt(0).toUpperCase() + entityType.slice(1)}</SheetTitle>
          <SheetClose />
        </SheetHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <SheetFooter>
              <Button type="submit" className="w-full rounded-lg bg-orange-500 hover:bg-orange-600 transition-colors">
                Save Changes
              </Button>
            </SheetFooter>
          </form>
