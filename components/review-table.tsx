...
export default function ReviewTable() {
  ...
  const movieId = 256; // Tentukan movieId dari konteks yang benar, misalnya dari props

  useEffect(() => {
    fetchReviews(movieId, sortOrder, sortType); // Panggil API ketika sortOrder atau sortType berubah
  }, [movieId, sortOrder, sortType]);

  useEffect(() => {
    if (!loading) {
      setReviewList(reviews); // Update reviewList dengan hasil dari API
    }
  }, [loading, reviews]);
      ...
      <Table>
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            <TableHead className="w-[100px] font-normal text-gray-300">Name</TableHead>
        ...
        <TableBody>
          {reviewList.map((review) => (
            <TableRow className="hover:bg-muted/5" key={review.id}>
              <TableCell className="font-medium text-gray-300">{review.user.username}</TableCell>
              <TableCell className="text-gray-300">{review.commentText}</TableCell>
              <TableCell className="text-center text-yellow-400">
                <Rating style={{ maxWidth: 65 }} value={review.rating} readOnly />
              </TableCell>
              <TableCell className="text-xs text-gray-300">{new Date(review.createdAt).toLocaleDateString()}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
